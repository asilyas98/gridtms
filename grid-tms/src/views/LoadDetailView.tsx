...
const formatForInput = (dateStr: string) => {
  if (!dateStr) return '';
  if (dateStr.length === 10) return `${dateStr}T08:00`;
...
const parseFromInput = (localStr: string) => {
  if (!localStr) return '';
...
export default function LoadDetailView({ loadId, onBack, navigate }: LoadDetailViewProps) {
  ...
  const confirmCancel = () => {
    updateLoad(loadId, { status: 'Archived' as any });
    setShowCancelConfirm(false);
    onBack();
  };

  const getReverseStatus = (status: string): string | null => {
    const map: Record<string, string> = {
      'Dispatched': 'Created',
      'At Pickup': 'Dispatched',
      'In Transit': 'At Pickup',
      'Loaded': 'At Pickup',
  ...
  const confirmDuplicate = () => {
    const newId = duplicateLoad(loadId);
    setShowDuplicatePrompt(false);
    if (newId) {
      navigate('loads', newId);
    }
  };
  ...
  const handleAddStop = () => {
    const newStop = {
      ...
      loadId: loadId,
      sequence: (load.stops?.length || 0) + 1,
      type: 'Stop Off' as const,
      locationName: 'Intermediate Stop',
    };
    updateLoad(loadId, { stops: [...(load.stops || []), newStop] });
  };

  const handleRemoveStop = (stopId: string) => {
    updateLoad(loadId, { stops: (load.stops || []).filter(s => s.id !== stopId) });
  };
    ...
    <div className="flex flex-col gap-6 animate-in slide-in-from-right-4 duration-500">
      ...
      <nav className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
           ...
           <button
             onClick={onBack}
             className="px-4 py-1.5 border border-blue-200 text-blue-600 bg-blue-50 rounded-lg text-xs font-bold hover:bg-blue-100 transition-all flex items-center gap-1.5"
           >
             <LogOut size={12} /> Close Load
           </button>
           <button
          ...
          <div className="bg-slate-50/50 rounded-xl p-6 border border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            {load.status === 'Created' && (
              <>
                ...
                <button
                  ...
                  className="w-full sm:w-auto px-6 py-3 bg-blue-600 text-white text-xs font-black uppercase tracking-widest rounded-xl hover:bg-blue-700 shadow-lg shadow-blue-600/20 transition-all text-center"
                >
                  Dispatch Load
                </button>
              </>
            ...
            {load.status === 'Dispatched' && (
              <>
                ...
                <div className="flex items-center gap-2">
                  {getReverseStatus(load.status) && (
                    <button
                      onClick={() => updateLoad(loadId, { status: getReverseStatus(load.status) as any })}
                      title={`Reverse to ${getReverseStatus(load.status)}`}
                      className="px-3 py-2.5 text-xs font-bold text-slate-500 border border-slate-200 rounded-xl hover:border-slate-400 hover:text-slate-700 transition-all flex items-center gap-1.5"
                    >
              ...
              <>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <motion.button
                    ...
                    className="w-full sm:w-auto px-6 py-3 bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-blue-700 shadow-lg shadow-blue-600/20 transition-all flex items-center justify-center gap-2"
                  >
            ...
            {load.status === 'Archived' && (
              <div className="flex items-center gap-3 w-full justify-center opacity-70 p-4">
                ...
                <h4 className="text-sm font-bold text-amber-600 uppercase tracking-widest">Load Archived</h4>
              </div>
            )}
          </div>
        </div>
        ...
        >
          <div className="flex justify-between items-center z-10 relative mb-1">
            ...
            {load.driverId && load.status !== 'Paid' && load.status !== 'Invoiced' && load.status !== 'Cancelled' && (
              <div className="absolute right-0 -top-1 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all">
                <button 
                  onClick={(e) => {
                    e.stopPropagation();
                    navigate('dispatch', loadId);
                  }}
                  className="text-[9px] font-bold text-blue-500 bg-blue-50 hover:bg-blue-100 px-2 py-0.5 rounded uppercase tracking-widest"
                >
                  Reassign
                </button>
                <button 
                  ...
                  className="text-[9px] font-bold text-red-500 bg-red-50 hover:bg-red-100 px-2 py-0.5 rounded transition-all uppercase tracking-widest"
                >
                  Unassign
                </button>
              </div>
          ...
          <div>
            ...
            <div className="flex flex-col gap-2">
              <div className="flex justify-between items-center">
                 ...
                 {isEditing ? (
                   <input type="datetime-local" className="text-xs w-32 border-b border-orange outline-none" defaultValue={formatForInput(load.pickupDate)} onChange={(e) => { if (e.target.value) updateLoad(loadId, { pickupDate: parseFromInput(e.target.value) }) }} />
                 ) : (
              ...
              <div className="flex justify-between items-center">
                 ...
                 {isEditing ? (
                   <div className="flex flex-col items-end gap-0.5">
                     <input
                       type="datetime-local"
                       className="text-xs w-32 border-b border-orange outline-none"
                       defaultValue={formatForInput(load.deliveryDate)}
                       min={formatForInput(load.pickupDate)}
                       onChange={(e) => {
                         if (e.target.value) {
                           const newDelivery = parseFromInput(e.target.value);
                           if (new Date(newDelivery) < new Date(load.pickupDate)) {
                             e.target.setCustomValidity('Delivery must be on or after pickup');
                             e.target.reportValidity();
                             return;
                           }
                           e.target.setCustomValidity('');
                           updateLoad(loadId, { deliveryDate: newDelivery });
                         }
                       }}
                     />
                   </div>
                           ...
                           <div className="flex items-center gap-1">
                             <input
                                ...
                                onChange={e => {
                                  ...
                                  updateLoad(loadId, { weight: Number.isNaN(val) ? 0 : Math.max(0, val) });
                                }}
                             />
              ...
              {activeTab === 'Stops' && (
                <div className="space-y-8 animate-in fade-in duration-500">
                   <div className="flex items-center justify-between mb-2">
                     ...
                     {isEditing && <button onClick={handleAddStop} className="text-[10px] font-bold text-orange uppercase tracking-wider hover:underline">+ Add Stop</button>}
                   </div>
                   <div className="space-y-6 relative">
                     ...
                     <div className="flex gap-6 relative">
                        ...
                        <div className="flex-1 bg-slate-50/50 rounded-xl p-4 border border-slate-100">
                          <div className="flex items-center justify-between mb-2">
                            ...
                            {isEditing ? (
                               <input type="datetime-local" className="text-[10px] border-b border-orange outline-none bg-transparent" defaultValue={formatForInput(load.pickupDate)} onChange={(e) => { if (e.target.value) updateLoad(loadId, { pickupDate: parseFromInput(e.target.value) }) }} />
                             ) : (
                     ...
                     {(load.stops || []).map((stop, idx) => (
                       <div className="flex gap-6 relative" key={stop.id}>
                          <div className="w-8 h-8 rounded-full bg-slate-300 text-white flex items-center justify-center font-bold text-xs shrink-0 z-10 border-4 border-white shadow-sm">{idx + 2}</div>
                          <div className="flex-1 bg-slate-50/50 rounded-xl p-4 border border-slate-100">
                            <div className="flex items-center justify-between mb-2">
                              <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">{stop.type}</span>
                              <span className="text-[10px] text-slate-400 font-mono">{stop.appointmentStart ? new Date(stop.appointmentStart).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) : 'TBD'}</span>
                            </div>
                            <div className="flex justify-between items-start">
                               <div>
                                 {isEditing ? (
                                   <input 
                                     className="text-sm font-bold text-navy-dark w-full bg-transparent border-b border-orange outline-none"
                                     value={stop.locationName || ''}
                                     onChange={e => {
                                       const newStops = [...(load.stops || [])];
                                       newStops[idx] = { ...newStops[idx], locationName: e.target.value };
                                       updateLoad(loadId, { stops: newStops });
                                     }}
                                     placeholder="Stop Location / Facility"
                                   />
                                 ) : (
                                   <h5 className="text-sm font-bold text-navy-dark tracking-tight">{stop.locationName}</h5>
                                 )}
                                 {stop.address && <p className="text-xs text-slate-500 mt-1">{stop.address.city}, {stop.address.state}</p>}
                               </div>
                               {isEditing && (
                                 <button onClick={() => handleRemoveStop(stop.id)} className="text-red-500 hover:text-red-700 p-1">
                                   <X size={14} />
                                 </button>
                               )}
                            </div>
                     ...
                     {/* Delivery Stop */}
                     <div className="flex gap-6 relative">
                        <div className="w-8 h-8 rounded-full bg-teal text-white flex items-center justify-center font-bold text-xs shrink-0 z-10 border-4 border-white shadow-sm">{(load.stops?.length || 0) + 2}</div>
                        <div className="flex-1 bg-slate-50/50 rounded-xl p-4 border border-slate-100">
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-[9px] font-bold text-teal uppercase tracking-widest">Delivery</span>
                            {isEditing ? (
                               <input type="datetime-local" className="text-[10px] border-b border-orange outline-none bg-transparent" defaultValue={formatForInput(load.deliveryDate)} onChange={(e) => { if (e.target.value) updateLoad(loadId, { deliveryDate: parseFromInput(e.target.value) }) }} />
                             ) : (
              ...
              {activeTab === 'Audit Trail' && (
                <div className="space-y-6 animate-in fade-in duration-500">
                   <div className="flex items-center gap-2 mb-4">
                     <Shield className="text-navy" size={18} />
                     <h3 className="text-sm font-bold text-navy-dark uppercase tracking-widest">Formal Audit Log</h3>
                   </div>
                   <AuditHistoryList entityType="Load" entityId={loadId} />
                </div>
              )}
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-navy-dark/40 backdrop-blur-sm">
          ...
          >
            <div className="px-6 py-5 border-b border-slate-100">
              <h2 className="text-lg font-black text-navy">Duplicate Load</h2>
              <p className="text-sm font-medium text-slate-500 mt-1">Are you sure you want to duplicate this load? All load details will be copied over with a new generated load number.</p>
            </div>
            <div className="p-4 bg-slate-50 flex justify-end gap-3 rounded-b-xl">
              <button
                onClick={() => setShowDuplicatePrompt(false)}
                className="px-4 py-2 bg-white border border-slate-200 rounded-lg text-xs font-bold text-navy hover:text-navy-light transition-all shadow-sm"
              >
                Cancel
              </button>
              <button
                onClick={confirmDuplicate}
                className="px-4 py-2 bg-navy text-white rounded-lg text-xs font-bold tracking-wide hover:bg-navy-light transition-all shadow-md shadow-navy/20"
              >
                Duplicate Load
              </button>
            </div>
      ...
      {showCancelConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-navy-dark/50 backdrop-blur-sm">
          <motion.div
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
          ...
          >
            <div className="px-6 py-5 border-b border-slate-100 flex items-start gap-4">
              ...
              <div>
                <h2 className="text-lg font-black text-navy">Cancel Load {load.loadNumber}?</h2>
                <p className="text-sm font-medium text-slate-500 mt-1">
                  This load will be <strong>cancelled and moved to the Archive tab</strong>. Archived loads are permanently excluded from settlements, reports, and invoicing. This action cannot be undone.
                </p>
              </div>
            </div>
            <div className="p-4 bg-slate-50 flex justify-end gap-3 rounded-b-xl">
              ...
              <button
                ...
                className="px-4 py-2 bg-red-500 text-white rounded-lg text-xs font-bold tracking-wide hover:bg-red-600 transition-all shadow-md shadow-red-500/20"
              >
                Yes, Cancel & Archive
              </button>
            </div>
