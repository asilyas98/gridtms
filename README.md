...
This is a Vite project, not a Next.js project. From this folder, use:
...
Do not run `next build` or `npx next build`. The root `package.json` intentionally
forwards build, development, and type-check commands to `frontend/`, preventing
TypeScript from scanning unrelated files elsewhere in the Windows user profile.
