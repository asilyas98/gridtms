...
export default function SettlementsView() {
  ...
  const handleConfirmBatchDeposit = async () => {
    ...
    fetch(`${API_BASE_URL}/tms/compliance_events`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ data: { id: `ach-${Date.now()}`, type: 'ACH_PAYOUT', status: 'executed', batchMethod, approvedSettleCount } })
    }).catch(err => console.warn('Failed to save ACH payout event to Supabase', err));
