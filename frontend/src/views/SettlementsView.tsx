...
export default function SettlementsView() {
  ...
  const loadSettlementsFromApi = async () => {
    try {
      if (!supabase) return;
      const { data, error } = await supabase.from('settlements').select('*').is('deleted_at', null);
      if (error) throw error;
      setApiSettlements(data as Settlement[]);
      setCalculationConnected(true);
    } catch (err) {
      console.warn("Error fetching settlements from Supabase.", err);
  ...
  const handleSaveSettlement = async () => {
    ...
    try {
      ...
      const dbPayload = {
        ...
        settlementNumber: editingSettlementId ? undefined : `SET-${2600 + apiSettlements.length + 1}`
      };
      
      const { error } = await supabase.from('settlements').upsert([dbPayload]);
      if (error) throw error;
      
      alert(
        editingSettlementId 
          ? `Settlement statement for ${targetDriver.name} is successfully updated!`
          : `Settlement statement for ${targetDriver.name} is successfully processed!`
      );
      ...
      alert("Failed to save settlement to database");
    }
  };
  ...
  const handleProgressSettlement = async (settle: Settlement) => {
    ...
    try {
      if (!supabase) return;
      const { error } = await supabase.from('settlements').update({ status: nextStatus }).eq('id', settle.id);
      if (error) throw error;
      loadSettlementsFromApi();
    } catch (err) {
      console.error("Error updating status:", err);
      alert("Sequential state validation active: Cannot skip steps.");
    }
  };
  ...
  const handleConfirmBatchDeposit = async () => {
    ...
    try {
      ...
      console.error("Error processing batch deposit:", err);
    }
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2.5">
          ...
          <button
            ...
            className="bg-blue-600 text-white hover:bg-blue-700 text-xs font-medium tracking-tight px-4.5 py-2.5 rounded-xl transition-all flex items-center gap-2 active:scale-97 shadow shadow-blue-600/20 cursor-pointer"
          >
            <Calculator size={14} />
