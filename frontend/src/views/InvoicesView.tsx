...
export default function InvoicesView({ navigate, selectedLoadId }: InvoicesViewProps) {
  ...
  const handleGenerateInvoice = (overrides: Partial<Invoice>, lineItems?: LoadLineItem[]) => {
    ...
    generateInvoice(invoiceBuilderLoadId, overrides);
  ...
  const handleConfirmPayment = async () => {
    ...
    if (paymentForm.attachment) {
      attachmentUrl = await uploadFileToSupabase(paymentForm.attachment, 'invoices') || undefined;
    }
    
    updateInvoice(paymentModalId, { status: 'Paid', attachment: attachmentUrl });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 space-y-6">
          {pendingInvoicing.length > 0 && (
            <div className={`tms-card transition-all duration-500 ${focusedLoad ? 'border-orange ring-2 ring-orange/10' : 'border-orange/20 bg-orange/5'}`}>
              <div className="p-4 border-b border-orange/10 flex items-center justify-between">
                <h4 className="text-xs font-bold text-orange uppercase tracking-wider flex items-center gap-2">
                  <Clock size={14} /> {focusedLoad ? 'Guided Workflow: Final Step' : 'Ready for Invoicing'}
                </h4>
                {focusedLoad && (
                  <span className="text-[10px] font-black text-orange animate-pulse px-2 py-0.5 bg-orange/10 rounded-full uppercase tracking-tighter">Action Required</span>
                )}
              </div>
              <div className="p-2">
                {focusedLoad && (
                  <div key={focusedLoad.id} className="flex items-center justify-between p-4 bg-orange/5 border border-orange/20 rounded-xl mb-2 transition-all shadow-sm text-black dark:text-white">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-full bg-orange text-white flex items-center justify-center font-bold text-xs shadow-md">
                        <Receipt size={20} />
                      </div>
                      <div>
                        <div className="font-mono text-sm font-black text-navy">{focusedLoad.loadNumber}</div>
                        <p className="text-[11px] font-bold text-slate-700">{getCustomerName(focusedLoad.customerId)}</p>
                        <p className="text-[10px] text-slate-400">Arrived at destination: {focusedLoad.deliveryDate}</p>
                      </div>
                    </div>
                    <button 
                      onClick={() => handleOpenBuilder(focusedLoad.id)}
                      className="px-6 py-3 bg-orange text-white text-xs font-black rounded-xl flex items-center gap-2 shadow-lg shadow-orange/20 hover:scale-105 transition-all uppercase tracking-widest cursor-pointer"
                    >
                      <Plus size={14} /> Generate & Send Invoice
                    </button>
                  </div>
                )}
                {otherPending.map(load => (
                  <div key={load.id} className="flex items-center justify-between p-3 hover:bg-white rounded-lg transition-all group">
                    <div className="flex items-center gap-4">
                      <div className="font-mono text-xs font-bold text-navy">{load.loadNumber}</div>
                      <div>
                        <p className="text-xs font-bold text-slate-700">{getCustomerName(load.customerId)}</p>
                        <p className="text-[10px] text-slate-400">Delivered on {load.deliveryDate}</p>
                      </div>
                    </div>
                    <button 
                      onClick={() => handleOpenBuilder(load.id)}
                      className="px-3 py-1.5 bg-orange text-white text-[10px] font-bold rounded-md flex items-center gap-1.5 shadow-sm opacity-80 group-hover:opacity-100 cursor-pointer"
                    >
            ...
            >
              ...
              <div className="p-6 bg-slate-50 border-t border-slate-100 flex gap-3">
                ...
                <button 
                   ...
                   className="flex-[2] py-4 bg-teal text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-teal/20 hover:bg-teal-light transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-wait"
                >
                  {isUploading ? 'Uploading & Closing...' : 'Confirm & Close Invoice'}
                </button>
              </div>
            </motion.div>
          </div>
