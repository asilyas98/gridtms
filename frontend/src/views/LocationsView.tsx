...
export default function LocationsView() {
  ...
  const triggerUploadSimulation = async () => {
    ...
    const url = await uploadFileToSupabase(currentSelectedFile, 'locations');
    if (!url) {
      setUploadProgress(null);
      setUploadSuccess(false);
      setUploadError('Upload failed. Verify the documents bucket and its locations upload policy.');
      return;
    }
  ...
  const handleCreateLocationSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditing && editingId) {
        const saved = await updateLocation(editingId, payload);
        if (!saved) throw new Error('Supabase did not update the location.');
      } else {
        savedLocation = await addLocation(payload);
        if (!savedLocation) throw new Error('Supabase did not save the location.');
      ...
      setLocationSaveError(error instanceof Error ? error.message : 'The location could not be saved to Supabase.');
    } finally {
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            {locationSaveError && (
              <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-xs font-semibold text-red-700">
                {locationSaveError} Check the location migration and Netlify function logs, then try again.
              </div>
            )}
            ...
            <div className="pt-4 flex items-center gap-4">
              ...
              >
                <Check size={16} /> {isSavingLocation ? 'Saving to Supabase…' : 'Save Facility details'}
              </button>
            </div>
