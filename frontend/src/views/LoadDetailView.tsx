...
export default function LoadDetailView({ loadId, onBack, navigate }: LoadDetailViewProps) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      setIsUploading(true);
      const url = await uploadFileToSupabase(file, 'loads');
      setIsUploading(false);
      
      addDocument(loadId, { name: newDocName || file.name, type: newDocType, url: url || undefined });
                      ...
                      <div className="flex flex-col md:flex-row items-start md:items-end gap-4">
                        <div className="flex-1 w-full space-y-4">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                           ...
                           <div 
                             onClick={!isUploading ? triggerFileUpload : undefined} 
                             className={`border-2 border-dashed ${isUploading ? 'border-slate-300 opacity-50 cursor-wait' : 'border-orange/30 hover:border-orange cursor-pointer'} bg-white rounded-xl p-6 text-center transition-all`}
                           >
                              <Printer size={24} className={`mx-auto mb-2 ${isUploading ? 'text-slate-400 animate-pulse' : 'text-orange'}`} />
                              <p className="text-xs font-bold text-navy">{isUploading ? 'Uploading file to secure storage...' : 'Click to browse or drag and drop a file'}</p>
                              <p className="text-[10px] text-slate-500 mt-1">{isUploading ? 'Please wait' : 'PDF, JPG, PNG up to 10MB'}</p>
                           </div>
                        </div>
