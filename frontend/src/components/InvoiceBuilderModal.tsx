...
export default function InvoiceBuilderModal({ load, customer, onClose, onGenerate, onAddDocumentToLoad }: Props) {
  ...
  const handleUploadSave = async () => {
    ...
    const url = await uploadFileToSupabase(uploadFile, 'invoices');
    setIsUploadingToSupabase(false);
    
    const newDoc = { id: `doc-new-${Date.now()}`, name: uploadName, type: uploadType, status: 'Valid' as any, expiryDate: '', url: url || undefined };
    setLocalDocs([...localDocs, newDoc]);
    setSelectedDocs([...selectedDocs, newDoc.id]);
    if (uploadSaveToLoad && onAddDocumentToLoad) {
      onAddDocumentToLoad(load.id, { name: uploadName, type: uploadType, url: url || undefined } as any);
    }
                        ...
                        {isUploading && (
                          <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-3 mt-2">
                            ...
                            <div className="flex items-center gap-2 pt-2">
                              <button onClick={() => setIsUploading(false)} className="flex-1 py-2 text-[10px] font-bold text-slate-500 hover:bg-slate-200 rounded-lg transition-all" disabled={isUploadingToSupabase}>Cancel</button>
                              <button onClick={handleUploadSave} disabled={isUploadingToSupabase} className="flex-[2] py-2 bg-orange text-white text-[10px] font-bold rounded-lg hover:bg-orange/90 transition-all shadow-sm disabled:opacity-50">
                                {isUploadingToSupabase ? 'Uploading...' : 'Save & Attach'}
                              </button>
                            </div>
                          </div>
                        )}
