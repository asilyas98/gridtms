...
export default function LoadWizard({ isOpen, onClose, navigate }: LoadWizardProps) {
  ...
  const handleNext = () => {
    if (step === 2 && formData.deliveryDate && formData.pickupDate && formData.deliveryDate < formData.pickupDate) {
      setDeliveryDateError('Delivery date must be on or after the pickup date.');
      return;
    }
    setDeliveryDateError('');
    setStep(s => s + 1);
  };
  const handleBack = () => { setDeliveryDateError(''); setStep(s => s - 1); };
              ...
              >
                <div className="space-y-4">
                  ...
                  <label className="block">
                    ...
                    <input
                      ...
                      onChange={e => { setFormData({ ...formData, pickupDate: e.target.value }); setDeliveryDateError(''); }}
                    />
                  </label>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <label className="block">
                    ...
                    <input
                      type="date"
                      className={`w-full bg-slate-50 border rounded-lg px-4 py-3 text-sm mt-1 ${deliveryDateError ? 'border-red-400 focus:ring-red-200' : 'border-slate-200'}`}
                      value={formData.deliveryDate}
                      min={formData.pickupDate}
                      onChange={e => { setFormData({ ...formData, deliveryDate: e.target.value }); setDeliveryDateError(''); }}
                    />
                    {deliveryDateError && <p className="text-[10px] text-red-500 font-bold mt-1">{deliveryDateError}</p>}
                  </label>
                </div>
              </motion.div>
            )}
                  ...
                  <label className="block">
                    ...
                    <input
                      ...
                      onChange={e => {
                        ...
                        setFormData({ ...formData, weight: Number.isNaN(val) ? 0 : Math.max(0, val) });
                      }}
                    />
                  </label>
                  <label className="block">
                  ...
                  <label className="block">
                    ...
                    <input
                      ...
                      onChange={e => {
                        ...
                        setFormData({ ...formData, rate: Number.isNaN(val) ? 0 : Math.max(0, val) });
                      }}
                    />
                  </label>
                  <label className="block">
                    ...
                    <input
                      ...
                      onChange={e => {
                        const val = e.target.value === '' ? 0 : parseFloat(e.target.value);
                        setFormData({ ...formData, miles: Number.isNaN(val) ? 0 : Math.max(0, val) });
                      }}
                    />
                  </label>
                </div>
